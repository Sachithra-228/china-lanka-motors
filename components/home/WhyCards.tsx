...
export function WhyCards({ items }: { items: WhyItem[] }) {
  ...
  const isExpanded = useCallback(
    (i: number) => opened.has(i) || hovered === i,
    [opened, hovered]
  );
    ...
    <div className="grid grid-cols-1 gap-3 sm:grid-cols-2 sm:gap-4">
      {first && (
        <article
          ...
          className={`relative flex h-[18rem] flex-col overflow-hidden border p-6 shadow-[0_18px_40px_rgba(0,0,0,0.2)] transition-all duration-300 sm:row-span-2 sm:h-[22rem] sm:p-6 md:h-[24rem] md:p-8 ${
        ...
        >
          ...
          <div
            className={`mt-2 flex min-h-0 flex-1 flex-col items-center justify-center transition-all duration-300 sm:mt-4 ${
              isExpanded(0) ? 'justify-start' : ''
            }`}
          >
            <div
              className={`font-unbounded text-[11px] font-extrabold uppercase tracking-[0.32em] transition-all duration-300 ${
            ...
            <h3
              className={`font-unbounded text-center font-semibold transition-all duration-300 ${
                isExpanded(0)
            ...
            <div
              className={`max-w-md text-center transition-opacity duration-300 ${
                isExpanded(0)
                  ? 'mt-3 opacity-100'
                  : 'absolute h-0 overflow-hidden opacity-0 pointer-events-none'
              }`}
      ...
      {rest.map((item, idx) => (
        <article
          ...
          onMouseLeave={() => setHovered(null)}
          className={`relative flex h-[11rem] flex-col overflow-hidden border p-4 shadow-[0_18px_40px_rgba(0,0,0,0.2)] transition-all duration-300 hover:-translate-y-0.5 sm:h-[12rem] sm:p-5 ${
        ...
        >
          ...
          <div className="relative flex min-h-0 flex-1 flex-col items-center justify-center transition-all duration-300">
            <div
              className={`font-unbounded text-[11px] font-extrabold uppercase tracking-[0.32em] transition-all duration-300 ${
            ...
            <div
              className={`max-w-xs text-center transition-opacity duration-300 ${
                isExpanded(idx + 1) ? 'mt-2 opacity-100' : 'absolute h-0 overflow-hidden opacity-0 pointer-events-none'
              }`}
      ...
      {last && (
        <article
          ...
          onMouseLeave={() => setHovered(null)}
          className={`relative flex h-[11rem] flex-col overflow-hidden border p-4 shadow-[0_18px_40px_rgba(0,0,0,0.2)] transition-all duration-300 hover:-translate-y-0.5 sm:col-span-2 sm:h-[12rem] sm:p-5 ${
          ...
          <div className="relative flex min-h-0 flex-1 flex-col items-center justify-center transition-all duration-300">
            ...
            <div
              className={`max-w-2xl text-center transition-opacity duration-300 ${
                isExpanded(items.length - 1) ? 'mt-2 opacity-100' : 'absolute h-0 overflow-hidden opacity-0 pointer-events-none'
              }`}
